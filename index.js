...
export function extractEntities(text) {
  const t = text.replace(/^\u7528\u6237/, '').replace(/^\s*пользовател\w*\s*/i, '').toLowerCase()
  const out = new Set()
  for (const w of t.match(/[a-z0-9\u0430-\u044f\u0451]+/g) || []) {
  ...
  const remember = async ({ text, kind, reason }) => {
    ...
    if (table.size > config.maxPreferences) {
      const weakest = [...table.entries()]
        ...
        .sort((a, b) => a[1] - b[1])[0]
      if (weakest) await table.delete(weakest[0])
    }
    return 'created'
  }
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args) {
        ...
        const outcome = await remember({ text, kind: args.kind, reason: String(args.reason || '').trim() })
